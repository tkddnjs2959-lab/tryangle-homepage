...
import { normalizedPhone, requestAddress, requestFingerprints } from '@/lib/inquiry-security';
...
export async function POST(req: Request) {
  ...
  const raw = body as { name?: unknown; contact?: unknown; message?: unknown; website?: unknown; attribution?: unknown; turnstileToken?: unknown; submissionId?: unknown; sessionId?: unknown; clarityReady?: unknown };
  ...
  if (phone.length < 10 || phone.length > 11) return NextResponse.json({ message: '연락처를 정확히 입력해주세요.' }, { status: 400 });
  if (name.length > 40 || contact.length > 30 || message.length > 5000) return NextResponse.json({ message: '입력 내용이 너무 깁니다.' }, { status: 400 });
